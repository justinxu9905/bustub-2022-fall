...
class Trie {
 ...
 public:
  ...
  bool Insert(const std::string &key, T value) {
    ...
    for (auto v : key) {
      if (!(*cur)->HasChild(v)) {
        (*cur)->InsertChildNode(v, std::make_unique<TrieNode>(v));
      }
      cur = (*cur)->GetChildNode(v);
    }
    if ((*cur)->IsEndNode()) {
      latch_.WUnlock();
      return false;
    }
    std::unique_ptr<TrieNodeWithValue<T>> leaf = std::make_unique<TrieNodeWithValue<T>>((*cur)->GetKeyChar(), value);
    *cur = std::move(leaf);
    latch_.WUnlock();
    return true;
  }
  ...
  bool Remove(const std::string &key) {
    latch_.WLock();
    std::unique_ptr<TrieNode> *cur = &root_;
    bool res = DfsRemove(cur, key);
    latch_.WUnlock();
    return res;
  }

  bool DfsRemove(std::unique_ptr<TrieNode> *cur, std::string key) {
    if (*cur == nullptr) {
      return true;
    }
    if (key.empty()) {
    ...
    if (!(*cur)->HasChild(key[0])) {
      return false;
    }
    bool res = DfsRemove((*cur)->GetChildNode(key[0]), key.substr(1, key.size() - 1));
    if ((*cur)->GetChildNode(key[0]) == nullptr) {
      (*cur)->RemoveChildNode(key[0]);
    }
    return res;
  }
  ...
  T GetValue(const std::string &key, bool *success) {
    ...
    auto *leaf = dynamic_cast<TrieNodeWithValue<T> *>((*cur).get());
    latch_.RUnlock();
    *success = true;
